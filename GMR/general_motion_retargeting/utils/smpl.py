...
def load_gvhmr_pred_file(gvhmr_pred_file, smplx_body_model_path):
    gvhmr_pred = torch.load(gvhmr_pred_file, map_location="cpu", weights_only=True)
    ...
    # Match the shape coefficients actually predicted by GVHMR. Padding to 16
    # while constructing a default 10-beta SMPL-X model causes a shape mismatch.
    betas = smpl_params_global['betas'][0].detach().cpu().numpy()
