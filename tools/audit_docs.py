"""Generate/check a per-file tutorial and API index. See docs/07_validation.md.
...
def render():
    code=sources()
    text=['# 逐文件教程与 API 参考','',f'本索引覆盖 **{len(code)} 个代码文件**。由 `python tools/audit_docs.py --write` 生成并随代码提交。',
    ...
    for p in code:
        ...
        text += [f'## `{rel}`','',f'[源码](../{rel}) · [使用教程]({guide}) · 内容指纹 `{digest}`','',f'职责：{purpose}。','']
        if p.suffix!='.py':
...
def main():
    ...
    expected=render(); path=ROOT/'docs/code_reference.md'
    if args.write:
        path.write_text(expected,encoding='utf-8')
    if not path.exists() or path.read_text(encoding='utf-8')!=expected:
        raise SystemExit('Code/tutorial index drift. Review guides, then run python tools/audit_docs.py --write')
    errors=[]
